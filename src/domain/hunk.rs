...
mod tests {
    ...
    #[test]
    fn test_extract_hunks_context_merges_close_changes() {
        // Two changes 3 lines apart, with context=2 their windows overlap → merge.
        // Lines: a B c d E f (changes at index 1 and 4, gap of 2 unchanged lines).
        // Context=2: hunk1 = 0..4, hunk2 = 2..6 → overlap → merged to 0..6.
        let old = "a\nb\nc\nd\ne\nf";
        let new = "a\nB\nc\nd\nE\nf";
        let diff = diff_lines(old, new);
        let rows = build_aligned_rows(&diff.ops);
        let hunks = extract_hunks(&rows, &diff.ops, 2);
        assert_eq!(hunks.len(), 1, "close changes should merge into one hunk");
        assert_eq!(hunks[0].row_range, 0..6);
    }

    #[test]
    fn test_extract_hunks_context_no_merge_far_apart() {
        // Two changes far apart — should remain separate even with context.
        // 10 lines, changes at index 1 and 8, context=2.
        let old = "a\nb\nc\nd\ne\nf\ng\nh\ni\nj";
        let new = "a\nB\nc\nd\ne\nf\ng\nh\nI\nj";
        let diff = diff_lines(old, new);
        let rows = build_aligned_rows(&diff.ops);
        let hunks = extract_hunks(&rows, &diff.ops, 2);
        assert_eq!(hunks.len(), 2, "far apart changes should stay separate");
    }
}
